...
final class BackgroundSyncService: ObservableObject {
    ...
    func start() {
        ...
        scheduler.tolerance = min(scheduler.interval / 2.0, 3_600)
    ...
    func trigger(reason: String, showToast: Bool = true) {
        guard let delegate = AppDelegate.shared else {
            SyncLogService.shared.logEvent(
                tag: "sync",
                level: "WARN",
                message: "Manual sync aborted (\(reason)): AppDelegate not ready"
            )
            return
        }
        ...
        guard !isSyncing else {
            ...
            SyncLogService.shared.logEvent(
                tag: "sync",
                level: "INFO",
                message: "Manual sync ignored (\(reason)) because a sync is in progress"
            )
            return
        }
        ...
        if FirebaseManager.shared.firestore == nil || Auth.auth().currentUser == nil {
            SyncLogService.shared.logEvent(
                tag: "sync",
                level: "WARN",
                message: "Manual sync aborted (\(reason)): not authenticated or Firebase not configured"
            )
